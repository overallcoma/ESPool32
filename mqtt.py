...
import network
import ujson
import uasyncio as asynicio
from binascii import hexlify

# MQTTClient class.
class MQTTClient:
    def __init__(self):        
        self._c = load_config("mqtt")
        self._c["client_id"] = hexlify(unique_id())
        self._c["password"] = str(self._c["password"]).encode()
        self._c["will"] = MQTTMessage(topic=self._c["topic"], message=self._c["will"])
        if not isinstance(self._c["will"], MQTTMessage):
        ...
        self._c["interface"] = network.WLAN(network.STA_IF)
        self._proto = None
    ...
    async def wifi_connect(self):
        ...
        station.active(True)
        station.connect(network_config["ssid"], network_config["password"])
        while station.status() == network.STAT_CONNECTING:  # Break out on fail or success.
            await asyncio.sleep_ms(200)
                
        self._c["interface"] = station        
    ...
    async def connect(self):
        ...
        if self._connect_coro is not None:
            loop.create_task(self._connect_coro(self))
            self._connect_coro = None  # FIXME: nasty...
        # Notify app that broker is connected
        if self._wifi_coro is not None:
            loop.create_task(self._wifi_coro(True))  # Notify app that Wifi is up
        # log.debug("connected")
    ...
    async def _reconnect(self, proto, why, detail="n/a"):
        if self._state == 1 and self._proto == proto:
            ...
            loop = asyncio.get_event_loop()
            if self._wifi_coro is not None:
                loop.create_task(self._wifi_coro(False))  # Notify application
